...
export function ChonkyDrive({ rootId, rootName, refreshKey, onUploaded }: { rootId: string; rootName: string; refreshKey?: number; onUploaded?: () => void }) {
  ...
  if (error && !current) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center bg-white p-12 text-center dark:bg-[#14141c]">
        <p className="text-sm font-medium text-ink">Couldn&apos;t load this folder</p>
        <p className="mt-1 text-sm text-red-600 dark:text-red-400">{error}</p>
        <button onClick={() => void reloadCurrent(true)} className="mt-3 rounded-full bg-[#101114] px-5 py-2 text-sm font-semibold text-white dark:bg-white dark:text-black">Retry</button>
      </div>
    );
  }
  ...
  if (!current) {
    return (
      <div className="flex min-h-0 flex-1 flex-col gap-3 bg-[#f7f7fa] p-4 dark:bg-[#0b0b11]">
        <div className="h-12 animate-pulse rounded-xl bg-white dark:bg-[#1c1c26]" />
        <div className="h-64 animate-pulse rounded-2xl bg-white dark:bg-[#1c1c26]" />
      </div>
    );
  }

  return (
    <div className="flex min-h-0 flex-1 flex-col bg-[#f7f7fa] dark:bg-[#0b0b11]" onDragOver={(e)=>{e.preventDefault();}} onDrop={handleDrop}>
      ...
      <div className="flex flex-1 min-h-0 flex-col overflow-hidden bg-white dark:bg-[#14141c]">
        ...
        <div className="flex items-center gap-3 border-t border-[#dedee5] bg-[#f7f7fa] px-4 py-2 text-xs text-[#686b82] dark:border-[#2a2a36] dark:bg-[#0b0b11] dark:text-[#a1a1aa]">
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-[#101114]/80 backdrop-blur-sm">
      <div className="flex items-center gap-3 bg-[#101114] px-4 py-3 text-white">
        ...
        <a href={url} download className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-[#101114]">Download</a>
      </div>
        ...
        <div className="max-h-full max-w-5xl w-full" onClick={(e)=>e.stopPropagation()}>
          {isImg ? (
            ...
            <div className="rounded-xl bg-white p-8 text-center shadow-2xl">
              ...
              <p className="font-mono text-xs text-[#686b82]">{mime}</p>
              <p className="mt-2 text-sm text-[#686b82]">Preview not available — download to view full file like Drive.</p>
              <a href={url} download className="mt-4 inline-flex rounded-full bg-[#7132f5] px-6 py-2.5 text-sm font-medium text-white">Download</a>
            </div>
          )}
        </div>
      </div>

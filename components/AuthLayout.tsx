...
/**
 * Split-screen auth layout — white form canvas + Kraken deep-purple brand
 * panel. Topped by the near-black announcement bar.
 */
  ...
  return (
    <main className="flex min-h-screen flex-col bg-canvas text-ink">
      ...
      <div className="grid flex-1 lg:grid-cols-[1fr_1.05fr]">
        ...
        {/* Brand panel side */}
        <div className="relative hidden overflow-hidden bg-[#2a1065] lg:block">
          {/* Fallback gradient (visible if the hero image fails to load) */}
          <div
            aria-hidden
            className="absolute inset-0 bg-gradient-to-br from-[#5b1ecf] via-[#2a1065] to-[#1c0b45]"
          />
          ...
          />
          {/* Kraken-purple overlay for text contrast + brand tint */}
          <div aria-hidden className="absolute inset-0 bg-[#2a1065]/55" />
          <div
            aria-hidden
          ...
          <div
            aria-hidden
            className="absolute -left-24 -top-24 h-96 w-96 animate-float-slow rounded-full bg-[#a855f7]/30 blur-[110px]"
          />
          <div
            aria-hidden
            className="absolute -bottom-32 -right-24 h-[28rem] w-[28rem] animate-float-slower rounded-full bg-[#7132f5]/35 blur-[130px]"
          />
...
/** Shared auth form primitives (Kraken text-input + 12px purple CTA). */
export function AuthField({
  ...
  return (
    <label className="flex flex-col gap-1.5 text-sm">
      ...
      <input
        {...props}
        className="h-11 rounded-xl border border-hairline bg-canvas px-3.5 text-[15px] text-ink outline-none transition-all duration-200 placeholder:text-stone hover:border-stone focus:border-kraken focus:ring-2 focus:ring-kraken/25"
      />
    </label>
  );
}
...
export function AuthSubmit({
  ...
  return (
    <button
      type="submit"
      disabled={loading}
      className="press flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-kraken text-sm font-semibold text-white transition-colors duration-200 hover:bg-kraken-deep disabled:cursor-wait disabled:opacity-60"
    >
      {loading && (
        <span
          aria-hidden

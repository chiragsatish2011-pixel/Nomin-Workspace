...
export function WorkspaceUnreadProvider({ children }: { children: React.ReactNode }) {
          ...
          try {
            ...
            if (shouldNotify && Notification.permission === "granted") {
              ...
              if (enabled) {
                ...
                triggerChatNotification({ title, body, tag: "nomin-chat-unread" });
                // Also dispatch in-app event so any toast can show
                window.dispatchEvent(new CustomEvent("nomin:chat-unread", { detail: { total } }));
              }
            }
          } catch {}
        }
  ...
  useEffect(() => {
    ...
    window.addEventListener("nomin:chat-unread", onCustom as EventListener);
    ...
    return () => {
      ...
      window.removeEventListener("nomin:chat-unread", onCustom as EventListener);

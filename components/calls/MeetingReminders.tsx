...
const SEEN_KEY = "nomin:scheduled-reminders:seen";
...
export function MeetingReminders({ userId }: { userId?: string | null }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nomin:meeting-reminder", onReminder);
    return () => window.removeEventListener("nomin:meeting-reminder", onReminder);
  }, []);
                ...
                if (browserAllowed) {
                  try {
                    new Notification(`You've been invited: ${occ.meeting.title}`, {
                      ...
                      icon: "/nomin-mark.png",
                      tag: `nomin-invite-${occ.meeting.id}`,
                    });
                  } catch {}
                }
              }
              ...
              if (browserAllowed) {
                try {
                  new Notification(`Meeting in 10 minutes: ${occ.meeting.title}`, {
                    ...
                    icon: "/nomin-mark.png",
                    tag: `nomin-reminder-${occ.meeting.id}-${occ.occurrenceStart}`,
                  });
                } catch {}
              }
              try {
                window.dispatchEvent(new CustomEvent("nomin:meeting-reminder", { detail: occ }));
              } catch {}
            }
          }
        }

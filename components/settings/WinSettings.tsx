...
import { useTheme, applyWorkspaceAccent, DEFAULT_ACCENT, type ThemeChoice } from "@/components/ThemeToggle";
import {
  BellIcon,
  InfoIcon,
  MonitorIcon,
...
export function WinSettings({ user }: { user: WinUser }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-118px)] flex-col bg-[#fbfbfb] dark:bg-[#0b0b11] md:flex-row">
      ...
      <aside className="w-full shrink-0 border-b border-hairline-soft bg-[#f3f3f3] p-3 dark:bg-[#101014] md:w-[280px] md:border-b-0 md:border-r">
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors ${
                    isActive ? "border border-hairline-soft bg-white text-ink shadow-sm dark:bg-[#1c1c26]" : "text-charcoal hover:bg-white/60 hover:text-ink dark:hover:bg-white/10"
                  }`}
                >
                  <span className={`grid h-7 w-7 place-items-center rounded-md ${isActive ? "bg-[#0078d4] text-white" : "border border-hairline-soft bg-white text-steel dark:bg-[#1c1c26]"} `}>
          ...
          <p className="mt-4 hidden px-2 font-mono text-[10px] uppercase tracking-wider text-stone md:block">Nomin Workspace · Settings</p>
        </aside>
...
function HomePanel({ user, onNav }: { user: WinUser; onNav: (id: NavId) => void }) {
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-xl border border-hairline bg-white p-4 shadow-sm dark:bg-[#14141c]">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.7fr_0.9fr]">
        {/* Left main card */}
        <div className="rounded-xl border border-hairline bg-white p-5 shadow-sm dark:bg-[#14141c]">
          <div className="flex items-center gap-2">
            <span className="grid h-6 w-6 place-items-center rounded bg-[#f3f3f3] text-xs">▦</span>
            <p className="text-sm font-semibold">Your Nomin account</p>
          </div>
        ...
        <div className="rounded-xl border border-hairline bg-white shadow-sm dark:bg-[#14141c]">
      ...
      <div className="rounded-xl border border-hairline bg-white p-4 shadow-sm dark:bg-[#14141c]">
...
function PersonalizeSwatches() {
  // Kraken Purple leads; the rest are the section-identity hues, all tuned
  // to the same scale so a custom accent never fights the brand.
  const colors = ["#7132f5", "#5741d8", "#2f6bf0", "#149e61", "#e0523c", "#101114"] as const;
  const [selected, setSelected] = useState<string>(() => {
    if (typeof window === "undefined") return DEFAULT_ACCENT;
    try {
      return window.localStorage.getItem("nomin:workspace:accent") || DEFAULT_ACCENT;
    } catch {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("nomin:workspace:accent", selected);
      applyWorkspaceAccent(selected);
    } catch {}
  }, [selected]);
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem("nomin:workspace:accent");
      applyWorkspaceAccent(saved || DEFAULT_ACCENT);
    } catch {}
  }, []);
  return (
...
function SystemPanel() {
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-hairline bg-white p-5 opacity-60 shadow-sm dark:bg-[#14141c]">
...
function AccountsPanel({ user }: { user: WinUser }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-steel">Your profile across Nomin — name, avatar, and team fields.</p>
      ...
      <div className="rounded-xl border border-hairline bg-white p-5 shadow-sm dark:bg-[#14141c]">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="font-mono text-xs uppercase tracking-wider text-steel">Role / Title at Nomin</label>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-hairline bg-white p-5 shadow-sm dark:bg-[#14141c]">
        ...
        <p className="mt-1 text-sm text-steel">
          Your workspace role is <span className="font-semibold text-ink">{user.role}</span>. Admins manage members and workspace settings; members can publish and comment. Your Nomin field/title (“{user.department || "—"} · {user.jobTitle || "—"}”) is just profile info, not a permission.
        </p>
      </div>
    </div>
  );
...
function AboutPanel({ user }: { user: WinUser }) {
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-hairline bg-white p-5 shadow-sm dark:bg-[#14141c]">
        <p className="font-semibold">Nomin Workspace</p>

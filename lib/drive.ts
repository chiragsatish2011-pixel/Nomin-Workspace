...
/**
 * Pre-rebrand folder name. Workspaces created before the Nomin rename keep
 * their files in this folder, so the lookup falls back to it rather than
 * silently creating an empty second root and orphaning every project.
 */
export const LEGACY_DRIVE_FOLDER_NAME = "Vaayu-Workspace-Projects";
...
/** Look one folder name up; null when the app can't see a folder by that name. */
async function findDriveFolderByName(
  ...
  return foundData?.files?.[0]?.id ?? null;
}

/**
 * Find the workspace folder by name, creating it on first use.
 *
 * When the current name has no folder of its own yet, the pre-rebrand name
 * is tried before anything is created — an existing workspace keeps its
 * files instead of starting over in a fresh, empty root.
 */
export async function ensureDriveFolder(
  accessToken: string,
  folderName: string = DRIVE_FOLDER_NAME
): Promise<string> {
  const existingId = await findDriveFolderByName(accessToken, folderName);

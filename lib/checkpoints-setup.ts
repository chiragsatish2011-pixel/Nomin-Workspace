...
export const CHECKPOINTS_SHEET_TITLE = "Nomin Checkpoints";
/**
 * Pre-rebrand title. Sheets created before the Nomin rename still carry it,
 * so discovery matches either title and existing checkpoint history is
 * reused instead of a second, empty spreadsheet being created.
 */
export const LEGACY_CHECKPOINTS_SHEET_TITLE = "Vaayu Checkpoints";
...
async function findAppSpreadsheetIds(
  ...
  const q = encodeURIComponent(
    `mimeType = 'application/vnd.google-apps.spreadsheet' and ` +
      `name = '${CHECKPOINTS_SHEET_TITLE}' and ` +
      `trashed = false`
  );

...
try {
  ...
  const au = await sql`INSERT INTO users (email, password_hash, role) VALUES (${tag + '_a@nomin.test'}, 'x', 'member') RETURNING id`;
  const bu = await sql`INSERT INTO users (email, password_hash, role) VALUES (${tag + '_b@nomin.test'}, 'x', 'member') RETURNING id`;
  const cu = await sql`INSERT INTO users (email, password_hash, role) VALUES (${tag + '_outsider@nomin.test'}, 'x', 'member') RETURNING id`;
  ...
  const du = await sql`INSERT INTO users (email, password_hash, role) VALUES (${tag + '_d@nomin.test'}, 'x', 'member') RETURNING id`;
  const D = du[0].id;
  ...
  await sql.query(`DELETE FROM users WHERE email LIKE '${tag}%@nomin.test'`);

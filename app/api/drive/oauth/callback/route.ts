...
function page(title: string, body: string): NextResponse {
  return new NextResponse(
    `<!doctype html><html lang="en"><head><meta charset="utf-8">` +
      ...
      `<style>body{font-family:ui-sans-serif,system-ui,sans-serif;background:#f7f7fa;color:#101114;display:flex;justify-content:center;padding:48px 16px;margin:0}` +
      `.card{background:#fff;border:1px solid #dedee5;border-radius:16px;padding:32px;max-width:640px;width:100%}` +
      `h1{font-size:22px;margin:0 0 12px}p{font-size:14px;line-height:1.6;color:#484b5e}` +
      `code{display:block;background:#1a0b3d;color:#fff;border-radius:8px;padding:14px 16px;font-size:13px;word-break:break-all;margin:12px 0}` +
      `.warn{background:#fef3f2;border:1px solid #d92d20;color:#b42318;border-radius:8px;padding:10px 14px;font-size:13px}` +
      `a{color:#7132f5}button{background:#7132f5;color:#fff;border:0;border-radius:12px;padding:13px 16px;font-size:14px;font-weight:600;cursor:pointer}</style></head>` +
